...
for (i in 1:20000) {
  if (i == 1) {
    ...
    random_point <- sample(triangle_list, 1) %>%
      unlist
...
starting_triangle <- triangle_list %>%
  lapply(function(z) tibble::tibble(x = z[1], y = z[2])) %>%
  dplyr::bind_rows()

zap_triangle <- test_list %>%
  lapply(function(z) tibble::tibble(x = z[1], y = z[2])) %>%
  dplyr::bind_rows() %>%
  dplyr::bind_rows(starting_triangle, .)
```
