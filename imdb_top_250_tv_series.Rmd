...
cat('
<meta property="og:title" content="IMDb Top 250 TV Series">
<meta property="og:description" content="Scraped episode ratings, seasons, and trends for the top 250 TV shows on IMDb.">
<meta property="og:url" content="https://cavandonohoe.github.io/imdb_top_250_tv_series.html">
...
```{r data}
all_eps_ratings_tib <- readr::read_csv(here::here("data", "top250_imdb_series.csv"))
```

```{r timestamp, results='asis'}
cat(sprintf(
  '<span class="data-timestamp">Data last updated: %s</span>\n',
  format(file.mtime(here::here("data", "top250_imdb_series.csv")), "%B %d, %Y")
))
```
...
max_ratings_per_series <- all_eps_ratings_tib %>%
  dplyr::group_by(imdb_id) %>%
  dplyr::filter(rating == max(rating, na.rm = TRUE)) %>%
  dplyr::ungroup()

max_ratings_per_series %>%
  dplyr::select(series_title, year = release_year, episode_name = title,
...
strong_closer <- all_eps_ratings_tib %>%
  dplyr::group_by(imdb_id) %>%
  dplyr::filter(dplyr::row_number() == max(dplyr::row_number(), na.rm = TRUE) &
           rating == max(rating, na.rm = TRUE)) %>%
             dplyr::ungroup()

strong_closer %>%
  dplyr::select(series_title, year = release_year, episode_name = title, season,
         episode, rating) %>%
  DT::datatable(filter = "top", extensions = "Buttons",
            options = list(pageLength = 500, dom = "Bfrtip", buttons = c("copy", "csv", "excel"),
                           scrollY = "400px", scrollX = TRUE))
...
strong_starter <- all_eps_ratings_tib %>%
  dplyr::group_by(imdb_id) %>%
  dplyr::filter(dplyr::row_number() == min(dplyr::row_number(), na.rm = TRUE) &
           rating == max(rating, na.rm = TRUE)) %>%
             dplyr::ungroup()
...
runs_over_9_streaks <- all_eps_ratings_tib %>%
  ...
  dplyr::filter(run_length >= 3) %>%
 # drop runs of 1 or 2 completely
  dplyr::mutate(
