...
if (!has_core) {
  ...
  cat(glue::glue(
    "### Overview\n",
    "{date_range_text}",
    "- Spend: {scales::dollar(kpis$spend)}\n",
    "- Impressions: {scales::comma(kpis$impressions)}\n",
    "- Clicks: {scales::comma(kpis$clicks)}\n",
    "- CTR: {scales::percent(kpis$ctr, accuracy = 0.1)}\n",
    "- Avg CPC: {scales::dollar(kpis$cpc)}\n",
    "- Conversions: {scales::comma(kpis$conversions)}\n",
    "- CVR: {scales::percent(kpis$cvr, accuracy = 0.1)}\n",
    "- CPL: {scales::dollar(kpis$cpl)}\n"
  ))
}
```
...
if (has_core && "campaign" %in% names(ads)) {
  ...
  DT::datatable(
    campaign_summary %>%
      dplyr::mutate(
        spend = scales::dollar(spend),
        ctr = scales::percent(ctr, accuracy = 0.1),
        cpc = scales::dollar(cpc),
        cvr = scales::percent(cvr, accuracy = 0.1),
        cpl = scales::dollar(cpl)
      ),
    rownames = FALSE,
