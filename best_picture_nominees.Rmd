...
cat('
<meta property="og:title" content="Best Picture Nominees">
<meta property="og:description" content="Oscar Best Picture nominee data — distributors, producers, and historical analysis since the first Academy Awards.">
<meta property="og:url" content="https://cavandonohoe.github.io/best_picture_nominees.html">
...
cat(sprintf(
  ...
  format(file.mtime(here::here("data", "best_picture_nominees.csv")), "%B %d, %Y")
))
```
...
distributors_plot_nominees <- best_pic_noms_final %>%
  ...
  dplyr::slice(1:15) %>%
  ggplot2::ggplot(ggplot2::aes(x = n, y= reorder(distributor, -n), text = all_titles,
    fill = distributor)) +
  ggplot2::geom_col()+
...
distributors_plot_nominees %>%
  plotly::ggplotly(tooltip = c("x", "text", "fill"), height = 800) %>%
...
<details class="table-accordion"><summary>Show distributors table</summary>
...
producers_plot_nominees <- best_pic_noms_final %>%
  ...
  dplyr::arrange(dplyr::desc(n)) %>%
  dplyr::slice(1:15) %>%
  ggplot2::ggplot(ggplot2::aes(x = n, y= reorder(producers, -n), text = all_titles,
    fill = producers)) +
  ggplot2::geom_col()+
  # ggplot2::labs(caption = "* = winners")+
  ggplot2::theme(axis.title.y=ggplot2::element_blank(),
...
<details class="table-accordion"><summary>Show producers table</summary>
...
<details class="table-accordion"><summary>Show all nominees table</summary>
