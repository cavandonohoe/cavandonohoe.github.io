...
top_250_html = url %>% rvest::read_html()

top_250_tib = top_250_html %>% 
  rvest::html_node("#main > div > span > div > div > div.lister > table") %>% 
  rvest::html_table() %>% tibble::as_tibble(.name_repair = "universal")
...
imdb_producers <- function(html_object) {
  html_object %>%
    ...
    unlist %>% stringr::str_trim() %>% stringr::str_squish() %>%
    `[` (.!="") %>%
    paste(collapse = "|")
}
...
#     rvest::html_nodes("section.ipc-page-background.ipc-page-background--base.TitlePage__StyledPageBackground-wzlr49-0.dDUGgO") %>% 
#     rvest::html_nodes("div > section > div") %>% 
#     rvest::html_nodes("div.TitleMainBelowTheFoldGroup__TitleMainPrimaryGroup-sc-1vpywau-1.btXiqv.ipc-page-grid__item.ipc-page-grid__item--span-2") %>% 
#     # rvest::html_nodes("section:nth-child(44) > div.styles__MetaDataContainer-sc-12uhu9s-0.cgqHBf > ul > li:nth-child(7)")
...
audience_score_fun = function(html_object){
  html_object %>% rvest::html_nodes("#topSection > div.thumbnail-scoreboard-wrap > score-board") %>%
...
directors_rot = function(html_object){
  director = html_object %>%
    ...
    dplyr::filter(title == "Director") %>% dplyr::pull(name)
...
movie_info_tib = movie_info %>% dplyr::bind_rows(.id = "title")
...
top_250_imdb_rot_tom = top_250_tib_clean %>% 
  ...
  dplyr::left_join(movie_info_tib %>% dplyr::filter(category == "Director") %>% dplyr::select(title, director = name)) %>% 
  dplyr::left_join(movie_info_tib %>% dplyr::filter(category == "Distributor") %>% dplyr::select(title, distributor = name)) %>%
  dplyr::left_join(movie_info_tib %>% dplyr::filter(category == "Production Co") %>% dplyr::select(title, production_co = name)) %>% 
