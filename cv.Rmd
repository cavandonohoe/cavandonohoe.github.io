...
resume_v1.1 <- resume %>%
  mutate(across(c(start, end), format_resume_date)) %>%
  mutate(across(starts_with("description"), as.character)) %>%
  mutate(description_1 = if_else(
    if_all(starts_with("description"), ~ is.na(.x) | .x == ""),
    "[keep]",
    description_1
  )) %>%
  mutate(start.date = as.Date(paste(start, "1", sep="-"), format="%B %Y-%d")) %>%
