...
random_sample_tib %>% dplyr::filter(sample %in% 1:5) %>% 
...
# average slope
random_sample_tib %>% dplyr::group_by(position) %>% 
  dplyr::summarise(ave_theta = mean(theta, na.rm = TRUE))

random_sample_tib %>% dplyr::pull(sample_sum) %>% unique
...
random_sample_tib %>% dplyr::pull(sample_sum) %>% mean
