...
# covid.cases_v4 %>% ggplot() + geom_line(aes(x=Date, y=Seven.Day.Average)) + geom_line(aes(x=Date, y=New.Cases)) + ggtitle("New Covid Cases")
...
Please note I only have data until `r max(covid.cases_v2$Date)`. The rolling average is the mean of that days cases as well as the 6 days prior. 
...
covid.cases_state_plot = covid.cases_state_v5 %>% ggplot(aes(x=Date, y=Seven.Day.Average, colour = state)) + geom_line() + 
  ...
  scale_y_continuous(labels = scales::comma) + scale_x_date(date_breaks = "1 month", date_labels = "%b %y") 
