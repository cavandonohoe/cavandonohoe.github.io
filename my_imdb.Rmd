...
nines_and_tens %>%
  ...
  dplyr::select(`My Rating` = `Your Rating`, Title, `IMDb Rating`, `Runtime (mins)`, Year, Genres, `Release Date`, Directors) %>%
...
```{r full_list}
my_ratings %>%
  dplyr::select(`My Rating` = `Your Rating`, Title, `Title Type`, `IMDb Rating`, `Runtime (mins)`, Year, Genres, `Release Date`, Directors) %>%
  dplyr::arrange(dplyr::desc(`My Rating`), dplyr::desc(`IMDb Rating`)) %>%
  DT::datatable(filter = "top", extensions = "Buttons",
            options = list(paging = FALSE, dom = "Bfrtip", buttons = c("copy", "csv", "excel"),
                           scrollY = "400px", scrollX = TRUE))
...
my_rating_plot = my_ratings %>%
  ...
  dplyr::group_by(title_sample, .add = TRUE) %>%
  dplyr::count() %>% dplyr::ungroup() %>%
  ggplot2::ggplot(ggplot2::aes(x = `Your Rating`, y = n, fill = `Title Type`, text = title_sample)) +
  ggplot2::geom_col() +
  ggplot2::scale_x_continuous(breaks = seq(1, 10, 1), limits = c(0.5, 10.5)) +
  ggplot2::theme_minimal() +
...
my_ratings %>%
  dplyr::select(my_rating = `Your Rating`, imdb_rating = `IMDb Rating`, title_type = `Title Type`) %>%
  ...
  ggplot2::ylab("My Rating") + ggplot2::xlab("IMDb Rating") + ggplot2::ggtitle("My Rating vs IMDB's Rating")
...
my_rating_vs_imdb = my_ratings %>%
  ...
  dplyr::group_by(my_rating_greater) %>% dplyr::count() %>% dplyr::ungroup() %>%
