...
```{r data}
all_eps_ratings_tib = read_csv("top250_imdb_series.csv") %>% select(-X1)
```
...
max_ratings_per_series = all_eps_ratings_tib %>%
  group_by(imdb_id) %>% 
  filter(user_rating == max(user_rating, na.rm = TRUE)) %>% 
  ungroup()

max_ratings_per_series %>% 
  select(series_name, year, episode_name = episode, season, episode = season_ep, rating = user_rating) %>% 
  datatable(filter = "top", options = list(paging = FALSE, scrollY = "400px", scrollX = TRUE))
```
...
strong_closer = all_eps_ratings_tib %>% group_by(imdb_id) %>% 
  filter(series_ep == max(series_ep, na.rm = TRUE) &
           user_rating == max(user_rating, na.rm = TRUE)) %>% ungroup()
...
shit_ending = all_eps_ratings_tib %>% group_by(imdb_id) %>% 
  filter(series_ep == max(series_ep, na.rm = TRUE) &
           user_rating == min(user_rating, na.rm = TRUE)) %>% ungroup()
