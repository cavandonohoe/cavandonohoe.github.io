...
producer_leaders_plot = top_250_imdb_rot_tom %>% tidyr::separate_rows(producers_imdb, sep = "\\|") %>%
  dplyr::group_by(producers_imdb) %>%
  dplyr::summarise(n = dplyr::n(), all_titles = paste(title, collapse = "\n")) %>%
  dplyr::arrange(dplyr::desc(n)) %>%
  dplyr::ungroup() %>%
  dplyr::slice(1:15) %>%
  ggplot2::ggplot(ggplot2::aes(y = reorder(producers_imdb, -n), x = n, text = all_titles, fill = producers_imdb))+
  ggplot2::geom_col() +
  ggplot2::theme(axis.title.y=ggplot2::element_blank(),
        axis.title.x=ggplot2::element_blank(),
        legend.position = "none") +
  ggplot2::ggtitle("Producer Leaderboard") +
  ggplot2::geom_text(ggplot2::aes(label = n), hjust = 0, position = ggplot2::position_dodge(width = 1))
  

producer_leaders_plot %>% plotly::ggplotly(tooltip = c("x", "text", "fill"))
