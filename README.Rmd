...
human commits (mine) vs. automated commits from the scheduled data
refresh workflows above. Regenerated each time `README.Rmd` is
knit.

```{r commit-activity, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 10, fig.height = 7, fig.path = "images/readme/", dev = "png", dpi = 110, fig.alt = "Two bar charts showing monthly commits and monthly lines changed (insertions + deletions) for this repository, split by human vs. bot author."}
...
df <- dplyr::bind_rows(commits) %>%
  dplyr::mutate(
    ...
    author_type = factor(
      ifelse(is_bot, "Automated (bots)", "Human"),
      levels = c("Human", "Automated (bots)")
    ),
...
monthly <- tidyr::expand_grid(
  ...
  dplyr::mutate(
    author_type = factor(author_type, levels = c("Human", "Automated (bots)"))
  )

palette <- c("Human" = "#2c7fb8", "Automated (bots)" = "#f0a202")
...
total_human <- sum(df$author_type == "Human")
