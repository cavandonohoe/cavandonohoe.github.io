...
```{r}
grad_list = googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1JwWeBjwwQHzmGgh8HPuO_0pghzemC3ikpLx_pXlQvsI/edit#gid=1184195844",
                       sheet = "Full Grad Class")

grad_list_clean = grad_list %>% dplyr::slice(-1) %>%
...
percent_accounted = grad_list_clean %>%
  dplyr::group_by(email_bool) %>% dplyr::count() %>%
  dplyr::ungroup() %>%
...
missing_list = grad_list_clean %>% dplyr::filter(!email_bool) %>%
