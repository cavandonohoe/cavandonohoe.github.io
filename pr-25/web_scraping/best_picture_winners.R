...
url <- paste0(
  ...
  "&sort=year,desc&count=100&view=simple"
)
...
box_office_from_imdb_page <- function(url) {
  gross_worldwide <- url %>%
    ...
    rvest::html_text() %>%
    tibble::as_tibble() %>%
      dplyr::filter(grepl(x=value, pattern = "Gross worldwide")) %>%
    dplyr::mutate(value = gsub(x=value, pattern = ".*Gross worldwide", replacement = "")) %>%
      dplyr::pull
...
oscar_winners_clean <- oscar_winners_tib %>%
  ...
  dplyr::mutate(oscar_year = dplyr::case_when(oscar_titles == "The Hurt Locker" ~ 2010,
                                ...
                                TRUE ~ oscar_year)) %>%
  dplyr::mutate(tconst = gsub(x=oscar_imdb_id,
    pattern = "https://www.imdb.com/title/|/\\?ref_=adv_li_tt",
                       replacement = ""))
