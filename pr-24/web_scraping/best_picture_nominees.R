...
year_tib = tibble::tibble(year_vec) %>% dplyr::filter(grepl(x=year_vec, pattern = "year")) %>%
...
for (url in year_tib$year_url) {
  oscar_html_list[[i]] = url %>% rvest::read_html()
  i= i+1
}
...
oscar_jscript_raw = function(html_object) {
  html_object %>% rvest::html_nodes("body > div > div > div > div > div > div > div > span") %>%
    rvest::html_text() %>% 
...
best_pic_noms_from_imdb = function(tib) {
  tib2 = tib %>% 
    ...
    dplyr::group_by(text2) %>% dplyr::mutate(row_num = dplyr::row_number()) %>% 
    dplyr::ungroup()
...
best_pic_noms = best_pic_noms_list %>% dplyr::bind_rows(.id = "year") %>%
...
for (url in best_pic_noms$imdb_url) {
  imdb_html_list[[i]] = url %>% rvest::read_html()
  i = i+1
}
...
i = 1
for (url in best_pic_noms$imdb_url) {
  imdb_prod_html_list[[i]] = url %>% file.path("companycredits?ref_=ttfc_sa_3") %>% rvest::read_html()
  i = i+1
}
...
best_pic_noms_final = best_pic_noms %>% dplyr::mutate(producers = producers) %>%
