...
box_office_oscars <- box_office %>%
  dplyr::full_join(oscars) %>%
  dplyr::mutate(oscar_winner_flag = ifelse(is.na(oscar_winner_flag), FALSE, oscar_winner_flag),
         box_office_top_1000_flag = ifelse(is.na(box_office_top_1000_flag), FALSE,
           box_office_top_1000_flag),
...
box_office_oscars_clean <- box_office_oscars %>%
  ...
  dplyr::mutate(legend = dplyr::case_when(
    box_office_top_1000_flag & oscar_winner_flag ~
      "Both Oscar Winning and Top 1000 Box Office",
...
overlap <- box_office_oscars_clean_split %>%
  dplyr::bind_rows() %>%
...
best_pic_top_box_boxplot <- box_office_oscars_clean_split %>%
  dplyr::bind_rows() %>%
  dplyr::filter(!is.na(lifetime_gross)) %>%
