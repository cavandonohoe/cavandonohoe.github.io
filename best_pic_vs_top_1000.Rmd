...
```{r data}
oscar_winners_clean = readr::read_csv(here::here("data", "best_picture_winners.csv"))

imdb_rank_ratings_clean = readr::read_csv(here::here("data", "top1000_box_office.csv"))
```

```{r clean}
box_office = imdb_rank_ratings_clean %>%
  dplyr::select(id, box_office_rank, title = Title,
         life_time_gross = `Lifetime Gross`, year = Year, imdb_rating) %>% 
  dplyr::mutate(box_office_top_1000_flag = TRUE)

oscars = oscar_winners_clean %>%
  dplyr::select(id = tconst, oscar_titles, year = movie_year, oscar_year,
         imdb_rating = oscar_imdb_ratings, life_time_gross_oscars = life_time_gross) %>% 
  dplyr::mutate(oscar_winner_flag = TRUE,
         life_time_gross_oscars = as.numeric(gsub('\\$|,', '', life_time_gross_oscars)),
         imdb_rating = as.numeric(imdb_rating))

box_office_oscars = box_office %>% dplyr::full_join(oscars) %>% 
  dplyr::mutate(oscar_winner_flag = ifelse(is.na(oscar_winner_flag), FALSE, oscar_winner_flag),
...
box_office_oscars_clean = box_office_oscars %>% 
  dplyr::select(title_final, lifetime_gross, imdb_rating, year,
         box_office_top_1000_flag, oscar_winner_flag, year) %>% 
  dplyr::mutate(legend = dplyr::case_when(box_office_top_1000_flag & oscar_winner_flag ~ "Both Oscar Winning and Top 1000 Box Office",
...
box_office_oscars_clean_split =
  list(box_office_oscars_clean %>%
         dplyr::filter(box_office_top_1000_flag) %>% 
         dplyr::mutate(legend = "Top 1000 Box Office"),
       box_office_oscars_clean %>%
         dplyr::filter(oscar_winner_flag) %>% 
         dplyr::mutate(legend = "Best Picture Winner"))


overlap = box_office_oscars_clean_split %>% dplyr::bind_rows() %>% 
  dplyr::filter(oscar_winner_flag & box_office_top_1000_flag) %>% 
  dplyr::distinct(title_final) %>% 
  dplyr::mutate(list = paste0(dplyr::row_number(), ". ", title_final, "\n"))

no_box_lifetime_gross = 
  box_office_oscars_clean_split %>% dplyr::bind_rows() %>% 
  dplyr::filter(is.na(lifetime_gross)) %>% 
  dplyr::mutate(list = paste0(dplyr::row_number(), ". ", title_final, "\n"))
```
...
best_pic_top_box_boxplot = box_office_oscars_clean_split %>% dplyr::bind_rows() %>%
  dplyr::filter(!is.na(lifetime_gross)) %>% 
  dplyr::mutate(label = paste(legend,
                      paste0("$", formatC(as.numeric(lifetime_gross), format = "f",
                                          big.mark = ",", drop0trailing = TRUE)),
                      title_final, year, sep = "\n")) %>%
  ggplot2::ggplot(ggplot2::aes(x = legend, y = lifetime_gross, fill = legend,
             label = label)) +
  ggplot2::geom_violin() +
  ggplot2::geom_point() +
  ggplot2::scale_fill_brewer(palette = "Dark2")+
  ggplot2::scale_y_continuous(labels = scales::dollar_format(), breaks = scales::pretty_breaks(n = 10)) +
  ggplot2::ylab("Lifetime Gross") +
  ggplot2::ggtitle("Lifetime Gross for Best Picture vs Top 1000 Box Office") +
  ggplot2::theme(axis.title.x = ggplot2::element_blank(),
        legend.position = "none")


best_pic_top_box_boxplot %>% plotly::ggplotly(tooltip = "label")
...
```{r no_box_office}
cat(no_box_lifetime_gross %>% dplyr::pull(list)) 
```

Very obviously, on average Best Picture Winners are a lagging a little bit at the box office (Best Pictures: `r scales::dollar(mean(oscars$life_time_gross_oscars, na.rm = TRUE))` vs Top Box Office: `r scales::dollar(mean(box_office$life_time_gross, na.rm = TRUE))`). 

## IMDb Rating for Best Picture vs Top 1000 Box Office

```{r imdb_rating violin}
best_pic_top_box_boxplot = box_office_oscars_clean_split %>% dplyr::bind_rows() %>%
  dplyr::filter(!is.na(imdb_rating)) %>%
  dplyr::mutate(label = paste(legend,
                       paste("IMDb Rating:", sprintf(imdb_rating, fmt = "%#.1f")),
                       title_final, year, sep = "\n")) %>%
  ggplot2::ggplot(ggplot2::aes(x = legend, y = imdb_rating, fill = legend,
             label = label)) +
  ggplot2::geom_violin() +
  ggplot2::geom_point() +
  ggplot2::scale_fill_brewer(palette = "Dark2")+
  ggplot2::scale_y_continuous(breaks = scales::pretty_breaks(n = 10)) +
  ggplot2::ylab("IMDb Rating") +
  ggplot2::ggtitle("IMDb Rating for Best Picture vs Top 1000 Box Office") +
  ggplot2::theme(axis.title.x = ggplot2::element_blank(),
        legend.position = "none")
