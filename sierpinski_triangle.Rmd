...
starting_triangle = triangle_list %>% lapply(function(z) tibble::tibble(x = z[1], y = z[2])) %>% dplyr::bind_rows()

zap_triangle = test_list %>% lapply(function(z) tibble::tibble(x = z[1], y = z[2])) %>% dplyr::bind_rows() %>% dplyr::bind_rows(starting_triangle, .)
```
...
zap_triangle %>%
  ...
  dplyr::filter(time <= 3) %>%
  ggplot2::ggplot(ggplot2::aes(x, y)) + ggplot2::geom_point() +
  ggplot2::theme_void()
```
