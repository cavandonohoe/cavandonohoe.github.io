...
```{r manifest}
# The show list lives in scripts/shows_manifest.R so this page and
# tv_decline_leaderboard.Rmd stay in sync. Add a show there.
...
summary_tbl <- shows %>%
  dplyr::mutate(
    data = purrr::map(slug, read_show_ratings),
...
render_one_show <- function(row) {
  data <- read_show_ratings(row$slug)
