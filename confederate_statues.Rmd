...
# find mode
max_statues = all_dates %>% dplyr::group_by(value3) %>%
  dplyr::count %>%
  dplyr::ungroup() %>%
...
all_dates_clean <- all_dates %>% dplyr::group_by(value3) %>%
  dplyr::count %>%
  dplyr::ungroup() %>%
  dplyr::mutate(date_bins = cut(value3, breaks = seq(min(value3), max(value3)+5, by = 5), right = FALSE),
...
confederate_statue_hist = all_dates_clean %>%
  ...
  ggplot2::scale_x_continuous(breaks = seq(min(all_dates_clean$date), max(all_dates_clean$date), by = 5)) +
...
confederate_statue_hist %>% plotly::ggplotly()
```
